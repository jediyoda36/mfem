...
double gt_func(const Vector &pt) {
   ...
   return cos(2*PI*pt[0]) * cos(2*PI*pt[1]);
}
...
// Dirichlet BC.
double dirichlet_func(const Vector &pt) {
   double x = pt[0];
   double y = pt[1];
   static const double PI = acos(-1.);
   return cos(2*PI*pt[0]) * cos(2*PI*pt[1]);
...
double rhs_func(const Vector &pt) {
   static const double PI = acos(-1.);
   return 8*PI*PI*cos(2*PI*pt[0]) * cos(2*PI*pt[1]);
}
...
int main(int argc, char *argv[])
{
   ...
   FunctionCoefficient dirichlet_bdr(dirichlet_func);
   ...
   x = 0.0;
   x.ProjectCoefficient(dirichlet_bdr);
